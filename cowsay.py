    ...
    def string_processor(txt, *args, **kwargs):
        ...
        if len(lines) == 1:
            ...
            if line_len <= max_line_len:
                ret_txt += "  " + "_" * line_len + "\n"
                ret_txt += " /" + " " * line_len + "\\" + "\n"
                ret_txt += "< " + lines[0] + " " * (line_len - len(lines[0]) + 1) + ">" + "\n"
                ret_txt += "  " + "=" * line_len + "\n"
            else:
            ...
            if all(len(line) <= max_line_len for line in lines):
                ...
                ret_txt += " /" + " " * line_len + "\\" + "\n"
                for line in lines:
                    ret_txt += "| " + line + " " * (line_len - len(line) + 1) + "|" + "\n"
                ret_txt += " \\" + " " * line_len + "/" + "\n"
                ret_txt += "  " + "=" * line_len + "\n"
            else:
                new_lines = []
...
@process_string
def __say(txt, base_character, max_line=max_line_len):
    for i in range(5, 9):
        txt += " " * (max_line + i) + "\\\n"
...
def beavis(txt):
    return __say(txt, Characters.beavis)


@apologize_if_fail_for("cheese")
def cheese(txt):
    return __say(txt, Characters.cheese)


@apologize_if_fail_for("daemon")
def daemon(txt):
    return __say(txt, Characters.daemon)


@apologize_if_fail_for("cow")
def cow(txt):
    return __say(txt, Characters.cow)


@apologize_if_fail_for("dragon")
def dragon(txt):
    return __say(txt, Characters.dragon)


@apologize_if_fail_for("ghostbusters")
def ghostbusters(txt):
    return __say(txt, Characters.ghostbusters)


@apologize_if_fail_for("kitty")
def kitty(txt):
    return __say(txt, Characters.kitty)


@apologize_if_fail_for("meow")
def meow(txt):
    return __say(txt, Characters.meow)


@apologize_if_fail_for("milk")
def milk(txt):
    return __say(txt, Characters.milk)


@apologize_if_fail_for("stegosaurus")
def stegosaurus(txt):
    return __say(txt, Characters.stegosaurus)


@apologize_if_fail_for("stimpy")
def stimpy(txt):
    return __say(txt, Characters.stimpy)


@apologize_if_fail_for("turkey")
def turkey(txt):
    return __say(txt, Characters.turkey)


@apologize_if_fail_for("turtle")
def turtle(txt):
    return __say(txt, Characters.turtle)


@apologize_if_fail_for("tux")
def tux(txt):
    return __say(txt, Characters.tux)
